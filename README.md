...
BATCH_SIZE = 32  # ResNet is heavy; 32 is a safe balance for Colab RAM
NUM_CLASSES = 7
train_dir = "/kaggle/input/facial-emotion-recognition-dataset/processed_data"
...
data_augmentation = tf.keras.Sequential([
    ...
    layers.RandomZoom(0.2),     # Increase zoom
    layers.RandomContrast(0.2), 
    layers.RandomBrightness(0.2),
    layers.RandomTranslation(height_factor=0.1, width_factor=0.1), # Add translation
])
...
x = base_model(x, training=False)
x = layers.GlobalAveragePooling2D()(x)
x = layers.Dense(256, activation='relu' , kernel_regularizer=regularizers.l2(0.01))(x)  # ResNet benefits from a wider dense layer
...
# SAVE
# -------------------------------
model.save('facial_emotion_3resnet50.keras')
print("ResNet50 Model Saved Successfully")
```
